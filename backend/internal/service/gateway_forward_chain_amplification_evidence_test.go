...
func buildClaudeCodeStyleBody(messages, bytesPerMessage int) []byte {
	...
	_, _ = b.WriteString(`{"model":"claude-sonnet-5","max_tokens":16384,"stream":true,"thinking":{"type":"enabled","budget_tokens":8192},`)
	_, _ = b.WriteString(`"system":[{"type":"text","text":"You are Claude Code, Anthropic's official CLI for Claude."},`)
	_, _ = b.WriteString(`{"type":"text","text":"` + strings.Repeat("Project context and instructions. ", 60) + `","cache_control":{"type":"ephemeral"}}],`)
	...
	_, _ = b.WriteString(`],"metadata":{"user_id":"user_abc123"},"messages":[`)
	for i := 0; i < messages; i++ {
		if i > 0 {
			_, _ = b.WriteString(",")
		}
		switch i % 4 {
		case 1: // assistant turn with tool_use
	...
	_, _ = b.WriteString(`,{"role":"user","content":[{"type":"text","text":"continue","cache_control":{"type":"ephemeral"}}]}]}`)
